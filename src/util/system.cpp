...
const char * const BITCOIN_CONF_FILENAME = "sceptrecoin.conf";
...
#else
    const char* pszModule = "sceptrecoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Sceptrecoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Sceptrecoin";
#else
    // Unix
    return pathRet / ".sceptrecoin";
#endif
#endif
}
